...
import {
    ...
    venusUpdateMusicalAnalysisSettings,
} from "@/apis/venus"
...
export async function updateMusicalAnalysisLevelAction(
    level: MusicalAnalysisLevel,
) {
    const { response, error } = await venusUpdateMusicalAnalysisSettings({
        body: {
            level: level,
        },
    })
    ...
    if (result === ActionOutcome.ok) {
        updateTag(VenusFetchTags.musicalAnalysisSettings)
    }

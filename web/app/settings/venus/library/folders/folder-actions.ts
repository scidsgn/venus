...
export async function addFolderAction(path: string) {
    const { response, error } = await venusAddFolder({
        body: {
            path,
        },
    })
    ...
    if (result === ActionOutcome.ok) {
        updateTag(VenusFetchTags.libraryFolders)
    }

    return result
}

export async function removeFolderAction(folderId: number) {
    const { response, error } = await venusDeleteFolder({
        path: {
            folder_id: folderId,
        },
    })

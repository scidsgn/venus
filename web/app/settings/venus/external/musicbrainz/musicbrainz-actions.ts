...
import {
    ...
    venusUpdateMusicbrainzSettings,
} from "@/apis/venus"
...
export async function updateMusicbrainzSettingsAction(
    request: MusicbrainzSettingsUpdateRequest,
) {
    const { response, error } = await venusUpdateMusicbrainzSettings({
        body: request,
    })
    const result = actionFromResponse(response, error?.code)

    if (result === ActionOutcome.ok) {
        updateTag(VenusFetchTags.musicbrainzSettings)
    }

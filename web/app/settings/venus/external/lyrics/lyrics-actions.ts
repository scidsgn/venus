...
export async function updateLyricsSettingsAction(
    request: LyricsSettingsUpdateRequest,
) {
    const { response, error } = await venusUpdateLyricsSettings({
        body: request,
    })
    const result = actionFromResponse(response, error?.code)

    if (result === ActionOutcome.ok) {
        updateTag(VenusFetchTags.lyricsSettings)
    }

...
import { ArtistWithDetailsDto, venusGetArtistRemixes } from "@/apis/venus"
...
type ArtistRemixesProps = {
    artist: ArtistWithDetailsDto
}

export const ArtistRemixes = async ({ artist }: ArtistRemixesProps) => {
    const result = await actionFromFetch(
        venusGetArtistRemixes({
            path: {
                artist_id: artist.id,
            },
        }),
